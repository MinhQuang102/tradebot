...
def detect_breakout(prices, highs, lows, period=20):
    ...
    latest_price = prices[-1]
    if latest_price > dynamic_resistance * 1.005:  # 0.5% above resistance
        return "Breakout Up"
    elif latest_price < dynamic_support * 0.995:  # 0.5% below support
...
async def main():
    try:
        ...
        app.add_handler(CommandHandler("analyze", analyze_command))  # Added analyze command
